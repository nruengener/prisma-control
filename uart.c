...
static FILE uart_output = FDEV_SETUP_STREAM(uart_putc_std, NULL, _FDEV_SETUP_WRITE);
FILE uart_input = FDEV_SETUP_STREAM(NULL, uart_getc_std, _FDEV_SETUP_READ);
...
unsigned int uart_getc(void)
{
    ...
    if ( UART_RxHead == UART_RxTail ) {  //**wenn hohe Flanke == tiefer Flanke-->keine Daten empfangen
...
int uart_getc_std(FILE *stream)
{
    unsigned char tmptail;
    char data;


    while ( UART_RxHead == UART_RxTail ); /* no data available */
...
int uart_putc_std(char data, FILE *stream)
{
    unsigned char tmphead;
